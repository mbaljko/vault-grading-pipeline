...
| ------- | ------------ | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ---------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------ | --------------------------------------- |
| Layer 1 | `indicator`  | `indicator_score`                   | `I_sid_cid_iid` where:<br>`sid` = submission identifier<br>`cid` = component identifier<br>`iid` = indicator identifier (two-character alphanumeric) | `I_PPP_SecA_I1` | evidence               | `indicator_evidence_scale`     | `evidence`, `partial_evidence`, `little_to_no_evidence`                                                            | a component of the student's submission |
| Layer 2 | `dimension`  | `dimension_score`                   | `D_sid_cid_did` where:<br>`sid` = submission identifier<br>`cid` = component identifier<br>`did` = dimension identifier (two-character alphanumeric) | `D_PPP_SecA_D1` | evidence               | `dimension_evidence_scale`     | `demonstrated`, `partially_demonstrated`, `little_to_no_demonstration`                                             | a component of the student's submission |
...
- SBOs: various `I_sid_cid_iid`
- Result: each student will have many, many `I_sid_cid_iid` scores.
#### Layer 1 scoring logic: 
- the score for each I_sid_cid_iid is determined by evidence found in the relevant AA
