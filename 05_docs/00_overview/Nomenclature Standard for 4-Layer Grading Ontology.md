...
### Layer 4
- One score for each `S_sid`, for each layer 4 grading unit.
- Result: each student will have one `S_sid` scores
...
- specifically, the score for each `S_sid` is determined by a mapping table that expresses the combination of `component_score` values that determine the `submission_score` valueo
