...
A calibration payload for a specific component is obtained by filtering the canonical dataset using the rule  
...
Example calibration dataset for Section A uses the filter rule  
...
- all scoring units correspond to the canonical structural unit `submission_id × component_id`
...
The final score produced by the rubric corresponds to the **Layer 4 submission Score-Bearing Object (SBO)** `submission_score`.
...
Input: `<ASSESSMENT_ID>_AssignmentPayloadSpec_v01`
...
```
#### Stage 0.2 Analytic Sub-space Identification
Some components ask students to perform **multiple distinct analytic moves**. These are treated as **analytic sub-spaces**.
An analytic sub-space is a **task-defined conceptual area** within a component response.
Analytic sub-spaces are derived from the component instructions and recorded in the **Submission Analytic Brief**. They are **not part of the scoring ontology** and do not appear in the Rubric Payload.
Example:
```text
Component: SectionAResponse
A1 accountability locus
A2 role boundary and responsibility hand-off
A3 professional obligations
```
Analytic sub-spaces serve as a **design scaffold** for indicator discovery and later dimension formation.
#### Deliverables
```text
...
#### Stage 0.3 Contrastive Pattern Discovery Pass
Before defining indicator SBO instances, use a **small calibration sample of real student responses** to discover contrastive response patterns.
This step is used to surface **observable signals actually present in student writing**, rather than relying only on the assignment description.
The purpose is to produce **candidate indicators and candidate dimensions** for each component.
These outputs remain **analytic hypotheses** at this stage.  
They do not become rubric structures until they are instantiated as SBO instances and stabilised through empirical testing in later stages.
#### Initial condition
The process begins once the **Submission Analytic Brief** and its component-specific analytic sub-spaces have been drafted.
A calibration sample is assembled:
```text
20–40 responses
```
...
```
#### Iterative testing process
Contrastive analysis is performed **within each analytic sub-space**, not across the response as a whole.
This is important for components that contain multiple analytic sub-spaces. A global contrastive analysis may surface differences in general writing quality or style rather than differences tied to the actual analytic tasks.
Operational workflow:
```text
1. identify analytic sub-spaces within the component
2. for each sub-space, prompt the model to identify contrastive response pairs
3. extract the textual signals that distinguish the response types
4. group those signals into candidate indicators
5. note early clusters that may later become candidate dimensions
```
Prompt concept for one analytic sub-space:
```text
...
These signals become **candidate indicators**. Signal clusters may also suggest **candidate dimensions**.
#### Exit condition
Stage 0 is complete when the Submission Analytic Brief contains, for each relevant component:
- a clear statement of analytic purpose
- identified analytic sub-spaces, where applicable
- a set of candidate indicators grounded in contrastive response evidence
- early candidate dimension groupings, where discernible
#### Deliverables
```text
<ASSESSMENT_ID>_SubmissionAnalyticBrief_v01
    └ component analytic sketches
