...
From this assignment, student submissions are expected to consist of **three structural pressure claim statements**, each following the AP3B Section B template.
...
3. If explicit markers are absent:
   - Segment using the **first three occurrences of `In this system`**
   - Also treat the AP3B template form `Claim 1: In this system`, `Claim 2: In this system`, `Claim 3: In this system` as claim starts where present
4. Each claim span should:
   - begin at its claim start marker (or `In this system`)
   - include the full claim statement as written by the student
   - end immediately before the next claim start
5. Ignore template labels or guidance text if present (e.g., `Claim statement (aim 40–50 words)`); extract only the claim content itself when separable.
6. If boundaries are ambiguous:
