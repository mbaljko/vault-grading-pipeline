...
generates section "4. Analytic Sub-space Identification" for the 
# . 
````
...
- The task is to derive analytic sub-spaces from the component instructions in the student-facing assignment materials, using the payload specification only for authoritative component structure and coverage. then output the section that will be inserted into the Submission Analytic Brief.
...
Three artefacts must be supplied verbatim using the delimiter `===`.
...
<ASSESSMENT_ID>_AssignmentMaterials
```

The Assignment Materials must include the student-facing analytic scaffold and may also include the case study.

Use:
- the Assignment Payload Spec for authoritative component structure
- the Submission Analytic Brief for assignment-level interpretive context
- the Assignment Materials for task decomposition
- the case study only to clarify case-specific analytic objects explicitly invoked by the task

If any artefact is missing, incomplete, malformed, or inconsistent, produce **no output**.

The Assignment Materials must include the student-facing analytic scaffold and may also include the case study.

Within the Assignment Materials:
- use the student-facing analytic scaffold as the primary authority for identifying analytic sub-spaces
- use the case study only to clarify analytic objects explicitly referenced in the scaffold
- do not derive analytic sub-spaces from background case detail that is not operationalised in the scaffold instructions
...
- **Derive analytic sub-spaces from the wording of the component instructions in the student-facing assignment materials, especially the analytic scaffold.**
    
- **Use** **<ASSESSMENT_ID>_AssignmentPayloadSpec_v01** **only to determine authoritative** **component_id** **structure and component coverage.**
    
- **Use the case study only where necessary to clarify the analytic object named in the component instructions.**
    
- **Do not derive additional sub-spaces solely from case details unless the instructions explicitly require attention to those details.**
...
### Rule 1A — Case study use is secondary and bounded

The case study may be used only to clarify the specific analytic object named in a component instruction.

The case study must not be used to:
- invent additional sub-spaces not required by the instructions
- split one instruction into multiple sub-spaces solely because the case contains rich detail
- import background entities that the student is not explicitly asked to analyse


### Rule 1B — Submission Analytic Brief consistency check

The Submission Analytic Brief may be used to confirm assignment-level interpretive context and the stated analytic purpose of each component.

Use it only to:
- confirm that proposed sub-spaces are compatible with the component’s stated analytic purpose
- maintain consistency of naming and assignment context

Do not use it to:
- invent additional analytic sub-spaces not required by the scaffold
- split scaffold tasks more finely than the instructions support
- override the task structure defined in the student-facing assignment materials

If a proposed sub-space would contradict the stated analytic purpose of a component in the Submission Analytic Brief, revise or remove it.
