...
def build_module_source(row: dict[str, str], payload: dict[str, object]) -> str:
	...
	return "\n".join(
		[
			...
			"",
			"def rule_matches(dimension_values: Mapping[str, str], rule: Mapping[str, object]) -> bool:",
			...
			"\treturn True",
			"",
			"def score_component(dimension_values: Mapping[str, str]) -> str:",
			"\tfor rule in DERIVATION_RULES:",
			"\t\tif rule_matches(dimension_values, rule):",
			...
			"",
			"def score_component_rows(rows: Iterable[Mapping[str, object]], dimension_id_field: str = 'dimension_id', value_field: str = 'evidence_status') -> str:",
			"\treturn score_component(build_dimension_value_map(rows, dimension_id_field=dimension_id_field, value_field=value_field))",
			"",
			"__all__ = [",
			"\t'ASSESSMENT_ID',",
			"\t'COMPONENT_ID',",
