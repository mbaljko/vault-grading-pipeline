...
| `operator_templates` / expanded instances | `runtime_family`             | `left_np_before_anchor`, `right_np_after_anchor_before_marker`, `span_after_marker_before_marker`, `local_effect_phrase_after_marker`, `status_only_anchor_detector`, `claim_text_passthrough_if_anchor`, `claim_text_passthrough_no_anchor` | If omitted, family is derived from `local_slot`/`output_mode`.                |
