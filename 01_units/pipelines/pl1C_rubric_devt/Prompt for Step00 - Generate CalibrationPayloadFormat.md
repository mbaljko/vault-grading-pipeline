...
The user supplies the authoritative inputs between the delimiter:
```
===
```
...
The artefact **must be enclosed inside a fenced Markdown block**.

- If the generated artefact contains any fenced code blocks, the outer fence must use four backticks:

\````

- The **first line inside the fenced block must be the file name expressed as a level-2 heading**:

```
## CAL_<ASSESSMENT_ID>_<COMPONENT_ID>_Step00_CalibrationPayloadFormat_v01
```

No text may appear before this heading inside the fence.
...
- assume neighbouring responses provide context
