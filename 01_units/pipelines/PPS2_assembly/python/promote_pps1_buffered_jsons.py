...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Annotate staged PPS1 JSON files with STUDENT_POOL and promote them into final destination directories.",
    )
    ...
    parser.add_argument("--all-output-dir", type=Path, required=True, help="Final destination directory for all JSON files.")
...
def main() -> int:
    ...
    clear_json_files(args.all_output_dir)
    ...
    for staged_path in sorted(args.staged_dir.glob("*.json")):
        ...
        student_pool = ["student_data_all"]

        if filename in sample_filenames:
            student_pool.append("student_data_sample")
        if email in section_m_emails:
            student_pool.append("student_data_SAS_SecM")
        if email in section_o_emails:
            student_pool.append("student_data_SAS_SecO")
